...
export const fetchAllProducts = async (token, resultsIndex) => {
  const query = {
    // "operationName": "Query",
    query: `query {
			paginationResolver(input: { limit: ${resultsPerQuery}, offset: ${
      resultsPerQuery * resultsIndex
    } }) {
				id
				name
			  }
  ...
  return graphQLQuery(token, query)
    .then((v) => {
      ...
      return {
        ...
        data: {
          allProducts: { ...v.paginationResolver },
          resultsIndex: resultsIndex + resultsPerQuery,
        },
      };
    })
